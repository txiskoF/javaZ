	...
	<div class="container m-4">
		...
		<div class="row">
			...
			<c:if test="${! empty libros }">
				<table class="table">
					...
					<c:forEach var="l" items="${libros }">
						<tr>
							...
							<td><a href="verLibro?id=${l.id}">${l.titulo }</a></td>
							<td><fmt:formatNumber type="currency">${l.precio }</fmt:formatNumber></td>
						</tr>
						</c:forEach>
					</tbody>
				</table>
				...
				<c:if test="${empty libros }">
				<div class="alert alert-info">No se han encontrado libros</div>
				</c:if>
			</div>
		</div>
	</div>
